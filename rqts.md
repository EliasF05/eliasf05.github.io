...
**What's happening?**<br>
This is only possible if we bias our search for possible reconstructions. We can do this by assuming that the unquantized time series was generated by an unknown autoregressive (AR(p)) process. Using the orange curve above, it's actually possible to identify the AR(p) model which generated the blue curve quite well. We can then use the identified model by applying particle smoothing to estimate the latent states (unquantized signal). At this point it is natural to ask whether this only works for AR(p) processes. Below, we see what happens when the AR(p)-assuming method to time series generated by an ARMA(p,q) (two poles, two zeros, all with equal coefficients) process.

Unfortunately, it is of course possible to construct time series on which the AR(p) assumption leads to very bad attempts at reconstruction. However, after having tried several model misspecifications as well as real-world data, I concluded that it generally does quite well on non-pathological stationary processes.
