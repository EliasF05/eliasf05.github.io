...
In words, aleatoric uncertainty quantifies the inherent uncertainty in the distribution over classes. For example, if we use the negative log-likelihood, $$\mathcal{l}(\theta, y) = -\log(\theta^Ty)$$, $$AU(Q)$$ is the expected entropy of $$\hat{\theta}$$.
